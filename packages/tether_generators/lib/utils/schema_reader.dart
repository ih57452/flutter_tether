...
/// Reads database schema information (tables, columns, foreign keys, comments)
/// from a PostgreSQL database using the provided configuration.
///
/// This class connects to the database specified in the [config],
/// queries the `information_schema` and `pg_catalog` to retrieve metadata,
/// and constructs [SupabaseTableInfo] objects representing the database structure.
/// It respects include/exclude rules defined in the configuration and fetches
/// column and foreign key details concurrently for efficiency.
class SchemaReader {
  ...
  Future<List<SupabaseTableInfo>> readTables() async {
    ...
    final List<SupabaseTableInfo> unsortedTables = [];

    // 1. Fetch ALL base table information
    ...
    // 2. Apply Exclusions based on ORIGINAL DB names
    ...
    // 3. Fetch details (Columns, FKs, Indexes) and build initial TableInfo list
    for (final row in includedBaseTables) {
      ...
      late final List<SupabaseIndexInfo> indexes; // <-- ADDED variable
      ...
      try {
        ...
        final results = await Future.wait([
          ...
          _readTableIndexes(schema, originalDbName), // <-- ADDED call
        ]);
        columns = results[0] as List<SupabaseColumnInfo>;
        foreignKeys = results[1] as List<SupabaseForeignKeyConstraint>;
        indexes = results[2] as List<SupabaseIndexInfo>; // <-- ASSIGN result
      } catch (e, s) {
        _logger.severe(
      ...
      // Add to the *temporary* unsorted list, including indexes
      unsortedTables.add(
        SupabaseTableInfo(
          name: toCamelCase(originalDbName), // Use utility function
          originalName: originalDbName,
          schema: schema,
          columns: columns,
          foreignKeys: foreignKeys,
          indexes: indexes, // <-- PASS indexes
          comment: comment,
        ),
      );
    }
  ...
  Future<List<SupabaseIndexInfo>> _readTableIndexes(
    ...
    try {
      ...
      for (final row in indexResult) {
        indexes.add(
          SupabaseIndexInfo.fromRow(row, toCamelCase),
        ); // Pass converter
      }
    } catch (e, s) {
      _logger.severe(
  ...
  Future<List<SupabaseColumnInfo>> _readTableColumns(
    ...
    for (final columnRow in columnsResult) {
      columns.add(SupabaseColumnInfo.fromRow(columnRow, toCamelCase));
    }
    return columns;
  }
