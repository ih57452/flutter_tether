...
class ModelGenerator {
  ...
  void _generateModelClass(StringBuffer buffer, SupabaseTableInfo table) {
    ...
    for (final column in table.columns) {
      final fieldName = column.localName.camelCase;
      final jsonKey = column.originalName;
    ...
    for (final fk in table.foreignKeys) {
      ...
      if (foreignTableInfo != null) {
        final relatedModelClassName = _getDartClassName(foreignTableInfo);
        final fieldName = _getFieldNameFromFkColumn(fk.originalColumns.first);
        final jsonKeyForRelatedObject =
            fieldName; // Assuming JSON key matches the field name
        buffer.writeln(
    ...
    for (final relation in reverseRelations) {
      ...
      buffer.writeln(
        '      $listFieldName: (json[\'$listFieldName\'] as List<dynamic>?)?.map((e) => $itemModelClassName.fromJson(e as Map<String, dynamic>)).toList(),',
      );
    }
    buffer.writeln('    );');
    buffer.writeln('  }');
    ...
    for (final column in table.columns) {
      final fieldName = column.localName.camelCase;
      final jsonKey = column.originalName; // Key within the 'json' map
      buffer.write('      $fieldName: ');
      _generateFromJsonConversion(
        buffer,
        column,
    ...
    for (final fk in table.foreignKeys) {
      ...
      if (foreignTableInfo != null) {
        ...
        // which uses the fieldName (derived from FK) as the key for the nested object.
        final jsonKeyForRelatedObject = fieldName;
        buffer.writeln(
          '      $fieldName: json[\'$jsonKeyForRelatedObject\'] == null ? null : $relatedModelClassName.fromJson(json[\'$jsonKeyForRelatedObject\'] as Map<String, dynamic>),',
        );
      }
    ...
    // Reverse relation fields are typically not included in this single JSON object structure
    // and remain null.
    for (final relation in reverseRelations) {
      final listFieldName = relation.fieldNameInThisModel;
      buffer.writeln(
        '      $listFieldName: null, // Reverse relations not populated from jsobjects',
      );
    }
    buffer.writeln('    );');
    buffer.writeln('  }');
    ...
    for (final fk in table.foreignKeys) {
      ...
      if (foreignTableInfo != null) {
        final fieldName = _getFieldNameFromFkColumn(fk.originalColumns.first);
        // The key for the related object in JSON should match how it's expected or fetched.
        // Using fieldName as the key.
    ...
    for (final relation in reverseRelations) {
      final listFieldName = relation.fieldNameInThisModel;
      buffer.writeln(
        '      \'$listFieldName\': $listFieldName?.map((e) => e.toJson()).toList(),',
      );
    }
    buffer.writeln('    };');
    buffer.writeln('  }');
